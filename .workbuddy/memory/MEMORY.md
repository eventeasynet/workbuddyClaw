...
### ⚠️ 潛在客戶情報強制規則（2026-06-16 新增，2026-06-16 更新）
- **⚠️ 第一步（必須先執行！）**：生成推廣郵件前，必須先用 WebFetch 讀取 https://eventeasy.net/guide_V1，了解 EventEasy.net 的實際功能
- **推廣郵件只能提及實際功能**（根據官方功能指南），絕對不能提及：
  - ❌ 白標服務（White Label）- EventEasy.net 沒有提供
  - ❌ API 对接 - EventEasy.net 沒有開放 API
  - ❌ 現場技術支持 - 這是高級增值服務，不是標準功能
- **不要在推廣內容內標明人數**（以免錯誤），改用「大量參與者」、「大型活動」等描述
- **署名規則**：
  - ❌ 不要用「創辦人」或「founder」
  - ✅ 用「EventEasy.net 團隊」或「The EventEasy.net Team」
- **聯絡電郵**：用 `eventeasynet@gmail.com`（不要用 `support@eventeasy.net`）
